...
export class Parser extends Reader {
    ...
    parseSegmentCmd(type, buf, file) {
        ...
        for (var i = 0, off = total; i < nsects; i++, off += sectSize) {
            ...
            sections.push({
                ...
                data: new Uint8Array(file.slice(offset, offset + size)),
            })
        }
    ...
    parseDysymtab(type, buf) {
        ...
        // NOTE:
        //     ilocalsym: The index into the symbol table for local symbols (symbols that are not meant for dynamic linking).
        //     nlocalsym: The number of local symbols.
        //     iextdefsym: The index into the symbol table for external definitions (symbols that are defined elsewhere but used in the current binary).
        //     nextdefsym: The number of external definitions.
        //     iundefsym: The index into the symbol table for undefined symbols (symbols that are declared but not defined within this binary, requiring resolution at runtime).
        //     nundefsym: The number of undefined symbols.
        return {
            type: type,
